...
void Animation::Render(D3DXVECTOR2 bottomPos, LPCAMERA camera, LPD3DXSPRITE spriteHandler, float opacity)
{
	if (this->frames.size() > 0 && curFrameID >= 0) {
		D3DXVECTOR2 renderPos;
		renderPos.x = bottomPos.x - this->Width() / 2;
		renderPos.y = bottomPos.y - this->Height();
		frames[curFrameID]->sprite->Draw(renderPos, camera, spriteHandler, opacity);
	}
	else
	{
		DebugOut(L"[Warnning]Animation::Render(): don't have sprite in sprites\n");
	}
}
...
void Animations::Render(D3DXVECTOR2 bottomPos, LPCAMERA camera, LPD3DXSPRITE spriteHandler, float opacity)
{
	if (animations.size() > 0 && curAnimationID >= 0) {
		animations[curAnimationID]->Render(bottomPos, camera, spriteHandler, opacity);
	}
	else
	{
		//DebugOut(L"[Warnning]Animations::Render(): don't have animation in animations");
	}
}
