...
LPCOEVENT Colision::SweptAABBEx(LPOBJECT obj, DWORD dt)
{
	...
	float time, nx, ny;
	float objvx, objvy;
	//objB = obj->BottomPosition()->x;
	objL = obj->CollisionBox().left;
	objT = obj->CollisionBox().top;
	objR = obj->CollisionBox().right;
	objB = obj->CollisionBox().bottom;
	objvx = obj->Velocity().x;
	objvy = obj->Velocity().y;
	float objdx = objvx * dt;
	float objdy = objvy * dt;
	float dx = character->Velocity().x * dt - objdx;
	float dy = character->Velocity().y * dt - objdy;
	chaL = character->CollisionBox().left;
	chaT = character->CollisionBox().top;
	chaR = character->CollisionBox().right;
	chaB = character->CollisionBox().bottom;
